...
namespace box {
    using interface::iforward;
    using interface::irefer;
 
    template<typename T>
    struct shared : iforward<T>, irefer<T> {
    private:
        ...
        struct __proxy {
            ...
            __proxy(T* _origin) : rc(1), origin(_origin) {
            }
    ...
    public:
        ...
        using wrapper = __proxy*;
        using ptr_type = T*;
 
        typename ref::left<T>::type operator*() override {
            return static_cast<typename ref::left<T>::type> (*box->get());
        }
        ptr_type operator->() override {
            return box->get();
        }
 
        shared(ptr_type ptr) : box(new __proxy(ptr)) {
        }
 
        shared(const shared<T>& shared)
        : box(shared.box->acquire()) {
        }
 
        ~shared() {
            if (box->release()->count() == 0) {
                delete box;
            }
        }
    private:
    ...
    shared<T> share(const Args&... args) {
